...
To prevent mathematical anomalies, this framework employs multi-level statistical metrics:
...
* **Byte-level KL Divergence ($D_{KL}(P || Q)$):** Measures the shift in raw byte/ASCII distribution. Crucial for detecting format-based anomalies.
* **Token Unigram & Bigram KL Divergence:** Mathematically captures distribution shifts caused by word replacement and structural reordering.
...
* **Information Entropy Difference ($\Delta H$):** Calculates the overall Shannon entropy shift pre- and post-embedding to detect unexpected randomness.
* **Robustness Testing (L2 & L3 Attacks):** * **L2 Attack (Format Cleaning):** Simulates channel noise like trailing space trimming and multiple-space collapse.
    * **L3 Attack (Semantic Rewriting):** Simulates advanced active wardens applying synonym fallback and paraphrasing.
...
The executable accepts standard command-line arguments to facilitate automated batch runs:
...
--file: Path to the cover text/log corpus.

--secret: The secret message to embed across all algorithms.

--iterations: Number of randomized runs to generate statistically sound means and 95% Confidence Intervals.

--outdir: Directory to save the output CSVs and stego-texts.
```
3. Interpreting the Output
results/stego_outputs/: Contains the physical .txt files generated by each algorithm during Iteration 0. Useful for manual inspection or passing to external 3rd-party steganalysis tools.

results/benchmark_summary.csv: Contains the aggregated academic metrics (Mean ± 95% CI). You can directly import this CSV into Python (Matplotlib/Seaborn) or LaTeX/pgfplots to generate comparison charts.

🧩 Extensibility: Adding a New Algorithm
To add a new steganography method to the benchmark:

Open src/compare_module.cpp.

Implement your embed_custom and extract_custom functions.

Register it in the run_academic_benchmark pipeline at the bottom of the file:
